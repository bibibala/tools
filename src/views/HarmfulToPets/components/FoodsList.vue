...
import foodsData from "../common/Foods.json";

// 展开所有食品数据，包括子分类
const allFoods = computed(() => {
    const foods = [];
    foodsData.household_products_risk_for_pets.forEach((category) => {
...
const totalPages = computed(() => {
    return Math.ceil(allFoods.value.length / itemsPerPage);
});
    ...
    <div class="foods-content">
        ...
        <div class="foods-articles">
            <article
                ...
                :key="food.id"
                class="food-article card"
            >
                <header class="article-header">
                    <h2 class="food-title-en">
                        {{ food.productName.english }}
                    </h2>
                    <h3 class="food-title-zh">
                        {{ food.productName.chinese }}
                    </h3>

                    <div
                        class="food-category"
                        v-if="
                            food.categoryName.english !==
                            food.productName.english
                        "
                    >
                        <span class="category-badge">
                            {{ food.categoryName.english }} /
                            {{ food.categoryName.chinese }}
                        </span>
                    </div>
                </header>
        ...
        <div class="pagination" v-if="totalPages > 1">
            <div class="pagination-info">
                ...
                {{ (currentPage - 1) * itemsPerPage + 1 }}-{{
                    Math.min(currentPage * itemsPerPage, allFoods.length)
                }}
                of {{ allFoods.length }} foods
            </div>
